...
class NPC:
    ...
    def update(self, barriers):
        """Move along the path with a delay between steps, and recalculate path."""
        if self.fsm == NPCState.MOVING and self.path:
            ...
            if self.frame_counter >= self.speed:  # Only move after a certain number of frames
                ...
                # Recalculate path based on updated state (barriers' movement)
                self.path = self.calculate_path(self.position, self.goal)
...
while running:
    ...
    # Update Game Logic
    npc.update(barriers)  # Pass barriers to update the NPC's path
    barriers.update()
